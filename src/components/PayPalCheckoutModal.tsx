  ...
  const createOrderHandler = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke(
        isSubscription ? "paypal-create-subscription" : "paypal-create-order",
        {
          body: isSubscription
            ? { amount: displayPrice }
            : { amount: displayPrice, tier: tier.id, description: `${tier.name} Premium Key` },
        }
      );
      if (fnError) throw new Error("Failed to create order");
      return isSubscription ? data.plan_id : data.order_id;
    } catch (err: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="p-6">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 text-muted-foreground hover:text-foreground transition-colors rounded-full p-1.5 hover:bg-secondary"
          >
            <X className="w-4 h-4" />
          </button>
          ...
          {success ? (
            <div className="text-center py-6">
              ...
              <button
                onClick={copyKey}
                className="w-full py-3 rounded-lg bg-primary text-primary-foreground font-semibold mb-3 hover:bg-primary/90 transition-colors"
              >
                Copy Key
              </button>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Your key is also saved to your dashboard if you're logged in.
              </p>
            </div>
          ) : (
            <>
              ...
              <div className="bg-secondary/50 border border-border rounded-lg p-3 mb-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <CheckCircle className="w-3.5 h-3.5 text-primary shrink-0" />
                    {isSubscription ? "Auto-renewal & priority support" : "Full duration access"}
                  </div>
                  <div className="flex items-center gap-2 text-xs">
                    <CheckCircle className="w-3.5 h-3.5 text-primary shrink-0" />
                    Discord VIP access
              ...
              <div className="flex items-center justify-center gap-3 mb-4">
                <div className="flex items-center gap-1.5 bg-secondary rounded-full px-3 py-1 text-[10px] text-muted-foreground">
              ...
              {error && (
                <div className="bg-destructive/10 border border-destructive/20 rounded-lg p-3 mb-4 text-sm text-destructive text-center">
                  {error}
                </div>
              )}
                ...
                >
                  {isSubscription ? (
                    <PayPalButtons
                      ...
                      createSubscription={createOrderHandler}
